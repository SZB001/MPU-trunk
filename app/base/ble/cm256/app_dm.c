 ...
 **                 mode: BLE Scan Mode(Active or Passive)
 **
 ** Returns         status: 0 if success / -1 otherwise
 **
 *******************************************************************************/
int app_dm_set_ble_scan_param(UINT16 interval, UINT16 window, UINT8 mode)
{
    ...
    APP_DEBUG1("Interval:%d (%dms) window:%d (%dms) mode:%d", interval, (int)((float)interval*0.625),
            window, (int)((float)window*0.625), mode);
...
int app_dm_set_ble_conn_param(tBSA_DM_BLE_CONN_PARAM *p_req)
{
    ...
    bt_config.ble_conn_param.preference = p_req->preference;
...
int app_dm_get_chip_id(void)
{
    ...
    case BSA_DM_CHIP_ID_20704A1:
        APP_INFO1("ChipId:BCM20704A1:%d", bt_config.chip_id);
        break;

    case BSA_DM_CHIP_ID_20704A2:
        APP_INFO1("ChipId:BCM20704A2:%d", bt_config.chip_id);
        break;
...
int app_dm_set_local_bt_name(void)
{
    ...
    /* Init config parameter */
    status = BSA_DmSetConfigInit(&bt_config);
	APP_INFO1("\tbd_addrName:0x%x:0x%x:0x%x:0x%x:0x%x:0x%x:", bt_config.bd_addr[0], bt_config.bd_addr[1], bt_config.bd_addr[2], bt_config.bd_addr[3], bt_config.bd_addr[4], bt_config.bd_addr[5]);
	//PRINTFBUF(bt_config.bd_addr, 6);
