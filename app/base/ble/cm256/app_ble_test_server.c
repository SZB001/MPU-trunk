...
int app_ble_test_server_start(void)
{
    ...
    /*Register BLE Server*/
    iRet = ql_ble_server_register(APP_BLE_TS_APP_UUID, ql_ble_server_profile_cback);
	if (-1 == iRet)
	{
		return -1;
	}
   ...
   ble_create_param.service_uuid.uu.uuid16 = 0x0001; 
    ...
    memcpy(ble_create_param.service_uuid.uu.uuid128,"\x9E\xCA\xDC\x24\x0E\xE5\xA9\xE0\x93\xF3\xA3\xB5\x01\x00\x40\x6E",16);
 #endif
  ...
  /* Add a characteristic */
    ...
    ble_addchar_param.char_uuid.uu.uuid16 = 0x0002;
#else
	ble_addchar_param.char_uuid.len = LEN_UUID_128;
	memcpy(ble_addchar_param.char_uuid.uu.uuid128,"\x9E\xCA\xDC\x24\x0E\xE5\xA9\xE0\x93\xF3\xA3\xB5\x02\x00\x40\x6E",16);
    
#endif
	
    //ble_addchar_param.perm = BSA_GATT_PERM_READ_ENCRYPTED | BSA_GATT_PERM_WRITE_ENCRYPTED;
...
#if 0		
    ble_addchar_param.char_uuid.len = 2;
    ble_addchar_param.char_uuid.uu.uuid16 = 0x0003;
#else	
	ble_addchar_param.char_uuid.len = LEN_UUID_128;
	memcpy(ble_addchar_param.char_uuid.uu.uuid128,"\x9E\xCA\xDC\x24\x0E\xE5\xA9\xE0\x93\xF3\xA3\xB5\x03\x00\x40\x6E",16);
#endif
    ble_addchar_param.perm = BSA_GATT_PERM_READ | BSA_GATT_PERM_WRITE;
    ...
    ble_addchar_param.char_uuid.len = 2;
    ble_addchar_param.char_uuid.uu.uuid16 = 0x2902;
    ble_addchar_param.perm = BSA_GATT_PERM_READ | BSA_GATT_PERM_WRITE;
    ble_addchar_param.property = 0;
    iRet = ql_ble_server_add_char(&ble_addchar_param);
	if (-1 == iRet)
	{
		return -1;
	...
	printf("********************************3\r\n");
    iRet = ql_ble_server_start_service(&ble_start_param);
	if (-1 == iRet)
	{
		return -1;
	}
