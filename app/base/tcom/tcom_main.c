                            ...
                            const unsigned char *msgbody)
{
    ...
    {
        log_e(LOG_TCOM,
              "tcom_enqueue_msg queue overflow, sender:0x%04x, receiver:0x%04x, msgid:%u, length:%u",
...
static int tcom_dequeue_msg(int queueidx, TCOM_MSG_HEADER *msghdr, unsigned char *msgbody)
{
    ...
    {
        log_e(LOG_TCOM, "tcom_dequeue_msg queue underflow, sender:%u, receiver:%u, msgid:%u, length:%u",
              msghdr->sender, msghdr->receiver, msghdr->msgid, msghdr->msglen);
                        ...
                        const unsigned char *msgbody)
{
    ...
    log_i(LOG_TCOM, "tcom_forward, reciver:0x%04x", receiver);
    ...
    if (0 != msghdr->msglen)
    {
        ...
        if (ret != 0)
        {
            log_e(LOG_TCOM, "tcom_enqueue_msg failed, ret:0x%08x,sender:%u,receiver:%u,msgid:%u", 
    ...
    if (ret != 0)
    {
        ...
        log_e(LOG_TCOM, "dev_write failed, ret:0x%08x,sender:%u,receiver:%u,msgid:%u", 
              ret, msghdr->receiver, msghdr->sender, msghdr->msgid);
...
static int tcom_proc_msg(void)
{
    ...
    if (ret != 0)
    {
        log_e(LOG_TCOM, "tcom_recv_msg failed, ret:0x%08x", ret);
        return TCOM_RECV_MSG_FAILED;
    }

    if (MPU_MID_TCOM != (msghdr.receiver & CPUID_MAIN_MID_MASK))
    {
        log_i(LOG_TCOM, "forwardmsg, sender:0x%04x, receiver:0x%04x, msgid:0x%08x, msglen:0x%08x",
              msghdr.sender, msghdr.receiver, msghdr.msgid, msghdr.msglen);
        /* forward it*/
        ...
        if (ret != 0)
        {
            log_e(LOG_TCOM, "tcom_forward failed, ret:0x%08x", ret);
            return ret;
        }
    }
...
void *tcom_main(void)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            log_e(LOG_TCOM, "tcom_main exit, error:%s", strerror(errno));
...
int tcom_run(void)
{
    ...
    if (ret != 0)
    {
        log_e(LOG_TCOM, "pthread_create failed, error:%s", strerror(errno));
...
return:       0 indicates success;
              others indicates failed
*****************************************************************/
int tcom_init(INIT_PHASE phase)
{
    int i, ret;
    ...
    switch (phase)
    {
        case INIT_PHASE_INSIDE:
            for (i = 0; i < MPU_APP_MID_COUNT; i++)
            {
                ...
                if (0 != ret)
                {
                    log_e(LOG_TCOM, "pipe failed, error:%s", strerror(errno));
                ...
                if (ret < 0)
                {
                    log_e(LOG_TCOM, "fcntl failed, error:%s", strerror(errno));
...
int tcom_send_msg(const TCOM_MSG_HEADER *msghdr, const void *msgbody)
{
    ...
    if (NULL == msghdr)
    {
        log_e(LOG_TCOM, "msghdr is NULL");
        return TCOM_INVALID_PARAMETER;
    }

    if (((msghdr->sender & CPUID_MASK) != 0) || ((msghdr->receiver & CPUID_MASK) != 0))
    {
        log_e(LOG_TCOM, "CPUID_MASK , sender or receiver is invalid,sender:0x%04x, receiver:0x%04x",
    ...
    {
        log_e(LOG_TCOM, "MPU_MAX_MID,sender or receiver is invalid,sender:0x%04x, receiver:0x%04x",
              msghdr->sender, msghdr->receiver);
        return TCOM_INVALID_PARAMETER;
    }
    ...
    {
        log_e(LOG_TCOM, "CPUID_MAIN_MID_MASK,sender or receiver is invalid,sender:0x%04x, receiver:0x%04x",
              msghdr->sender, msghdr->receiver);
        return TCOM_INVALID_PARAMETER;
    }
    ...
    {
        log_e(LOG_TCOM, "msg header is invalid, sender:0x%04x, receiver:0x%04x, msgid:0x%08x",
    ...
    {
        log_e(LOG_TCOM, "msg length is invalid senderid:%04x,receiverid:%04x,msgid:%d", msghdr->sender,
...
int tcom_get_read_fd(unsigned short main_mid)
{
    ...
    {
        log_e(LOG_TCOM, "main_mid(0x%08x) is invalid", main_mid);
        return TCOM_INVALID_PARAMETER;
    }
    ...
    {
        log_e(LOG_TCOM, "TCOM fd can not be read");
        return TCOM_INVALID_PARAMETER;
    }
...
int tcom_recv_msg(unsigned short main_mid, TCOM_MSG_HEADER *msghdr, unsigned char *msgbody)
{
    ...
    if (NULL == msgbody)
    {
        log_e(LOG_TCOM, "msgbody is NULL");
        pthread_mutex_unlock(&tcom_queue_mutex[index]);
        return TCOM_INVALID_PARAMETER;
    }
