...
void PP_ChargeCtrl_init(void)
{
	...
	//读取配置
	len = 16;
	res = cfg_get_para(CFG_ITEM_HOZON_TSP_RMTAPPOINT,&PP_rmtCharge_AppointBook.validFlg,&len);
	...
	switch(ctrlstyle)
	{
		case RMTCTRL_TSP:
		{
			...
			{//预约
				...
				//保存预约记录
				(void)cfg_set_para(CFG_ITEM_HOZON_TSP_RMTAPPOINT,&PP_rmtCharge_AppointBook.validFlg,16);
			}
