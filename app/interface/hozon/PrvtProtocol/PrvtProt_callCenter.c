 ...
 int PrvtProt_CC_Answercall(void)
{
	...
	uint32_t len = 32;
	int ret;
	if(assist_get_call_status() == 1)   //TBOX����
	{
		at_get_call_incoming_telno(number);//��ȡ����ĺ���
		...
		if(strcmp(number,xcall) == 0) //ECALL����
		{
			audio_basic_ICall();
			answercall();//����
			ecall_flag = 1; //ECALLͨ����־
		...
		if(strcmp(number,xcall) == 0) //BCALL����
		{
			bcall_flag = 1; //BCALLͨ����־
			return 0;
		}
		...
		if(strcmp(number,xcall) == 0) //ICALL����
		{
			icall_flag = 1; //ICALLͨ����־
			return 0;
		}
