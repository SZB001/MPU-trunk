...
typedef struct
{
    ...
    char
    smsCenterAddr[QL_SMS_MSG_CENTER_ADDR_NUM_MAX_LEN];  ///SMS center address number string.
    char
    timestamp[QL_SMS_TIMESTAMP_MAX_BYTES];    ///Message time stamp (in text mode). string format: "yy/MM/dd,hh:mm:ss+/-TimeZone"
    size_t
    userdataLen;  ///Number of characters for text and UCS2 messages, or the length of the data in bytes for raw binary messages
